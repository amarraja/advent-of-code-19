...
defmodule Test do
  ...
  test "day5 first examples" do
    assert [3500 | _] = Day13.run("1,9,10,3,2,3,11,0,99,30,40,50", 1)
    assert [2 | _] = Day13.run("1,0,0,0,99", 1)
  end

  test "commands" do
    assert {:mul, 3, []} = Day13.extract_command(2)
    assert {:mul, 3, [:immediate]} = Day13.extract_command(102)
    assert {:mul, 3, [:positional, :immediate]} = Day13.extract_command(1002)
  end
  ...
  test "day 5 part2 examples" do
    ...
    assert [_, [999] | _] = Day13.run(program, 7)
    assert [_, [1000] | _] = Day13.run(program, 8)
    assert [_, [1001] | _] = Day13.run(program, 9)
  end

  test "day9 part 1 examples" do
    [_, output | _] = Day13.run("109,1,204,-1,1001,100,1,100,1008,100,16,101,1006,101,0,99")
    assert output == [109, 1, 204, -1, 1001, 100, 1, 100, 1008, 100, 16, 101, 1006, 101, 0, 99]

    [_, [num] | _] = Day13.run("1102,34915192,34915192,7,4,7,99,0")
